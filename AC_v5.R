...
names(j_STOP_num)[-1] <-
  gsub("^",
       "Num_",
       names(j_STOP_num)[-1])
# j_STOP_num_TRAIN$Test_set_id <- NULL
for (col in names(j_STOP_num)) {
  j_STOP_num[, (col) := replace(get(col), is.na(get(col)) | 
                                  is.infinite(get(col)), 0)]
}
# j_STOP_num <-
#   merge(j_STOP_num, classification, all.x = TRUE)
# j_STOP_num <-
#   j_STOP_num[match(classification$Customer_ID, j_STOP_num$Customer_ID),]
# write.csv(x = j_STOP_num, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/j_STOP_num.csv')
# 找到 j_STOP_num 数据表中 Num_SOP 为 0 的那一列的 Customer_ID
j_STOP_num_Customer_ID <- j_STOP_num$Customer_ID[j_STOP_num$Num_SOP == 0]
classification_N <- classification[!classification$Customer_ID %in% j_STOP_num_Customer_ID, ]
# j_STOP_num <- j_STOP_num[!classification$Customer_ID %in% j_STOP_num_Customer_ID, ]
...
SOP_j_sales_organization <- SOP_j_sales_organization %>%
  replace_na(list(
    'sales_organization_NA' = 0,
    'sales_organization_A' = 0,
    'sales_organization_B' = 0
  ))
names(SOP_j_sales_organization)[-1] <-
  gsub("^", "SOP_", names(SOP_j_sales_organization)[-1])
# SOP_j_sales_organization <-
#   merge(SOP_j_sales_organization, classification_N, all.x = TRUE)
# SOP_j_sales_organization <-
#   SOP_j_sales_organization[match(classification_N$Customer_ID, SOP_j_sales_organization$Customer_ID),]
# write.csv(x = SOP_j_sales_organization, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_sales_organization.csv')
...
setDT(SOP_j_sales_order_num)
for (col in names(SOP_j_sales_order_num)) {
  SOP_j_sales_order_num[, (col) := replace(get(col), is.na(get(col)) |
                                           is.infinite(get(col)), 0)]
}
# SOP_j_sales_order_num <-
#   merge(SOP_j_sales_order_num, classification_N, all.x = TRUE)
# SOP_j_sales_order_num <-
#   SOP_j_sales_order_num[match(classification_N$Customer_ID, SOP_j_sales_order_num$Customer_ID), ]
# write.csv(x = SOP_j_sales_order_num, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_sales_order_num.csv')
...
setDT(SOP_NV_Variance)
for (col in names(SOP_NV_Variance)) {
  SOP_NV_Variance[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
}
# SOP_NV_Variance <-
#   merge(SOP_NV_Variance, classification_N, all.x = TRUE)
# setDT(SOP_NV_Variance)
# SOP_NV_Variance <-
#   SOP_NV_Variance[match(classification_N$Customer_ID, SOP_NV_Variance$Customer_ID), ]

names(SOP_NI_Variance)[-1] <-
  gsub("^", "STP_NI_", names(SOP_NI_Variance)[-1])
setDT(SOP_NI_Variance)
for (col in names(SOP_NI_Variance)) {
  SOP_NI_Variance[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
}
# SOP_NI_Variance <-
#   merge(SOP_NI_Variance, classification_N, all.x = TRUE)
# SOP_NI_Variance <-
#   SOP_NI_Variance[match(classification_N$Customer_ID, SOP_NI_Variance$Customer_ID), ]
# write.csv(x = SOP_NV_Variance, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_NV_Variance.csv')
...
# SOP_j_net_value <-
#   merge(SOP_j_net_value, classification_N, all.x = TRUE)
# SOP_j_net_value <-
#   SOP_j_net_value[match(classification_N$Customer_ID, SOP_j_net_value$Customer_ID), ]
# write.csv(x = SOP_j_net_value, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_net_value.csv')
...
names(SOP_NI_Variance)[-1] <-
  gsub("^", "SOP_NI_", names(SOP_NI_Variance)[-1])
setDT(SOP_NI_Variance)
for (col in names(SOP_NI_Variance)) {
  SOP_NI_Variance[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
...
# SOP_NI_Variance <-
#   SOP_NI_Variance[match(classification_N$Customer_ID, SOP_NI_Variance$Customer_ID), ]
# write.csv(x = SOP_NI_Variance, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_NI_Variance.csv')
SOP_j_Num_Items <- SOP_j_Num_Items %>%
  group_by(Customer_ID) %>%
  summarise(Total_Num_Items = sum(Num_Items))
names(SOP_j_Num_Items)[-1] <-
  gsub("^", "SOP_", names(SOP_j_Num_Items)[-1])
setDT(SOP_j_Num_Items)
for (col in names(SOP_j_Num_Items)) {
  SOP_j_Num_Items[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
}
# SOP_j_Num_Items <-
#   merge(SOP_j_Num_Items, classification_N, all.x = TRUE)
# SOP_j_Num_Items <-
#   SOP_j_Num_Items[match(classification_N$Customer_ID, SOP_j_Num_Items$Customer_ID), ]
# write.csv(x = SOP_j_Num_Items, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Num_Items.csv')
...
setDT(SOP_j_c_m_num_items)
for (col in names(SOP_j_c_m_num_items)) {
  SOP_j_c_m_num_items[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
}
# SOP_j_c_m_num_items <-
#   merge(SOP_j_c_m_num_items, classification_N, all.x = TRUE)
# SOP_j_c_m_num_items <-
#   SOP_j_c_m_num_items[match(classification_N$Customer_ID, SOP_j_c_m_num_items$Customer_ID), ]
...
setDT(SOP_j_c_m_Net_Value)
for (col in names(SOP_j_c_m_Net_Value)) {
  SOP_j_c_m_Net_Value[, (col) := replace(get(col), is.na(get(col)) |
                                           is.infinite(get(col)), 0)]
}
# SOP_j_c_m_Net_Value <-
#   merge(SOP_j_c_m_Net_Value, classification_N, all.x = TRUE)
# SOP_j_c_m_Net_Value <-
#   SOP_j_c_m_Net_Value[match(classification_N$Customer_ID, SOP_j_c_m_Net_Value$Customer_ID), ]
...
setDT(SOP_j_c_m_BU_A)
for (col in names(SOP_j_c_m_BU_A)) {
  SOP_j_c_m_BU_A[, (col) := replace(get(col), is.na(get(col)) |
                                           is.infinite(get(col)), 0)]
}
# SOP_j_c_m_BU_A <-
#   merge(SOP_j_c_m_BU_A, classification_N, all.x = TRUE)
# SOP_j_c_m_BU_A <-
#   SOP_j_c_m_BU_A[match(classification_N$Customer_ID, SOP_j_c_m_BU_A$Customer_ID), ]
...
setDT(SOP_j_c_m_BU_B)
for (col in names(SOP_j_c_m_BU_B)) {
  SOP_j_c_m_BU_B[, (col) := replace(get(col), is.na(get(col)) |
                                      is.infinite(get(col)), 0)]
}
# SOP_j_c_m_BU_B <-
#   merge(SOP_j_c_m_BU_B, classification_N, all.x = TRUE)
# SOP_j_c_m_BU_B <-
#   SOP_j_c_m_BU_B[match(classification_N$Customer_ID, SOP_j_c_m_BU_B$Customer_ID), ]
...
setDT(SOP_j_c_m_BU_C)
for (col in names(SOP_j_c_m_BU_C)) {
  SOP_j_c_m_BU_C[, (col) := replace(get(col), is.na(get(col)) |
                                      is.infinite(get(col)), 0)]
}
# SOP_j_c_m_BU_C <-
#   merge(SOP_j_c_m_BU_C, classification_N, all.x = TRUE)
# SOP_j_c_m_BU_C <-
#   SOP_j_c_m_BU_C[match(classification_N$Customer_ID, SOP_j_c_m_BU_C$Customer_ID), ]
...
setDT(SOP_j_c_m_Net_Value_A)
for (col in names(SOP_j_c_m_Net_Value_A)) {
  SOP_j_c_m_Net_Value_A[, (col) := replace(get(col), is.na(get(col)) |
                                       is.infinite(get(col)), 0)]
}
# SOP_j_c_m_Net_Value_A <-
#   merge(SOP_j_c_m_Net_Value_A, classification_N, all.x = TRUE)
# SOP_j_c_m_Net_Value_A <-
#   SOP_j_c_m_Net_Value_A[match(classification_N$Customer_ID, SOP_j_c_m_Net_Value_A$Customer_ID), ]
# SOP_j_c_m_Net_Value_A <- SOP_j_c_m_Net_Value_A[, -c(330,331)]
# write.csv(x = SOP_j_c_m_Net_Value_A, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_Net_Value_A.csv')
...
setDT(SOP_j_Document_Type)
for (col in names(SOP_j_Document_Type)) {
  SOP_j_Document_Type[, (col) := replace(get(col), is.na(get(col)) |
                                             is.infinite(get(col)), 0)]
}
# SOP_j_Document_Type <-
#   merge(SOP_j_Document_Type, classification_N, all.x = TRUE)
# SOP_j_Document_Type <-
#   SOP_j_Document_Type[match(classification_N$Customer_ID, SOP_j_Document_Type$Customer_ID), ]
# write.csv(x = SOP_j_Document_Type, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Document_Type.csv')
...
setDT(SOP_j_Delivery)
for (col in names(SOP_j_Delivery)) {
  SOP_j_Delivery[, (col) := replace(get(col), is.na(get(col)) |
                                           is.infinite(get(col)), 0)]
}
# SOP_j_Delivery <-
#   merge(SOP_j_Delivery, classification_N, all.x = TRUE)
# SOP_j_Delivery <-
#   SOP_j_Delivery[match(classification_N$Customer_ID, SOP_j_Delivery$Customer_ID), ]
# write.csv(x = SOP_j_Delivery, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Delivery.csv')
...
setDT(SOP_j_Business_Unit)
for (col in names(SOP_j_Business_Unit)) {
  SOP_j_Business_Unit[, (col) := replace(get(col), is.na(get(col)) |
                                      is.infinite(get(col)), 0)]
}
# SOP_j_Business_Unit <-
#   merge(SOP_j_Business_Unit, classification_N, all.x = TRUE)
# SOP_j_Business_Unit <-
#   SOP_j_Business_Unit[match(classification_N$Customer_ID, SOP_j_Business_Unit$Customer_ID), ]
# write.csv(x = SOP_j_Business_Unit, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Business_Unit.csv')
...
setDT(SOP_j_Material_Class)
for (col in names(SOP_j_Material_Class)) {
  SOP_j_Material_Class[, (col) := replace(get(col), is.na(get(col)) |
                                           is.infinite(get(col)), 0)]
}
# SOP_j_Material_Class <-
#   merge(SOP_j_Material_Class, classification_N, all.x = TRUE)
# SOP_j_Material_Class <-
#   SOP_j_Material_Class[match(classification_N$Customer_ID, SOP_j_Material_Class$Customer_ID), ]
# write.csv(x = SOP_j_Material_Class, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Material_Class.csv')
...
SOP_j_MERGE <-
  merge(j_STOP_num, SOP_j_Business_Unit, by = 'Customer_ID', all.y = TRUE)
SOP_j_MERGE <-
  merge(SOP_j_MERGE, SOP_j_c_m_BU_A, by = 'Customer_ID')
SOP_j_MERGE <-
  merge(SOP_j_MERGE, SOP_j_c_m_BU_B, by = 'Customer_ID')
SOP_j_MERGE <-
  merge(SOP_j_MERGE, SOP_j_c_m_BU_C, by = 'Customer_ID')
SOP_j_MERGE <-
  merge(SOP_j_MERGE, SOP_j_c_m_Net_Value, by = 'Customer_ID')
SOP_j_MERGE <-
...
Feature <-
  merge(SOP_j_MERGE, classification_N, all.x = TRUE)
Feature <-
  Feature[match(classification_N$Customer_ID, Feature$Customer_ID), ]

# Feature <-
#   merge(Feature, classification, by = "Customer_ID", all = TRUE)
write.csv(x = SOP_j_MERGE, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/Feature.csv')
write.csv(x = SOP_j_Business_Unit, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Business_Unit.csv')
write.csv(x = SOP_j_c_m_BU_A, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_BU_A.csv')
write.csv(x = SOP_j_c_m_BU_B, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_BU_B.csv')
write.csv(x = SOP_j_c_m_BU_C, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_BU_C.csv')
write.csv(x = SOP_j_c_m_Net_Value, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_Net_Value.csv')
write.csv(x = SOP_j_c_m_Net_Value_A, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_c_m_Net_Value_A.csv')
write.csv(x = SOP_j_c_m_num_items, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/SOP_j_c_m_num_items.csv')
write.csv(x = SOP_j_Delivery, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Delivery.csv')
write.csv(x = SOP_j_Document_Type, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Document_Type.csv')
write.csv(x = SOP_j_Material_Class, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_Material_Class.csv')
write.csv(x = SOP_j_net_value, file = 'E:/TUM/22WS/Business Analytics and Machine Learning (IN2028)/AC_Project/training_data/OUTPUT/SOP_j_net_value.csv')
